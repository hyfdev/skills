...
- Before drafting GitHub prose, establish that the user who sent the review request has actually reviewed or explicitly supervised the result, then resolve that user's GitHub handle. Use a handle stated in that request, or the authenticated `gh` account only when it is clearly the same user; do not infer it from a display name. Sign everything the review publishes as that user's review and disclose the exact model that assisted it; never call it an AI review. End the review body with a signature line such as `— Review by @<requesting-user-handle>, assisted by <exact-model>`. If the sender's supervision or handle is not known, do not publish the review until the user provides or confirms it. This signature is the standing exception to keeping reviewer-process metadata out of author-facing prose, and the prose-only pass above never removes it. When an approval would otherwise have no body, the signature line alone is the body. Append the same line to any comment posted outside a review, such as a file-level comment or a thread reply, since those have no review body to carry it. Inline comments inside a review need no individual signature.
